...
| N8a | Leave a drift notice unreviewed and check drift twice more: once with no further change, once after changing another setting. | Still **one** row for the device. After the unchanged check, the Tasks page's "Last seen by" time moves and nothing else does. After the change, the strip's field count updates, and a review note says "last changed …" as well as "last confirmed …". |
