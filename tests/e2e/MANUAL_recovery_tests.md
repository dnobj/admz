...
and the **real factory reset → setup** (which mutates a device). Since
2026-09-23 a queued recovery never provisions by itself: it raises a `setup`
notice, and the device is set up through the chat, behind an approval.
...
| B1 | On a `needs_setup` device detail page | Amber banner: *"This device is factory-defaulted (needs setup). Setting it up gives root the fleet root password and creates ADMZ's own admz account, after you approve. Or decommission it."* with a **Set it up** button |
| B2 | Click **Queue setup notice** | Success message; the pending list shows *"⏳ reprovision · …expires in 24h"* with a **Cancel** button |
...
| C1 | Chat: "I want to factory reset device `<id>`" | Agent ASKS the follow-up (set it up again / remove / leave) before proceeding; gates the reset itself |
| C2 | Chat: "Set `<online id>` up again when it comes back from a reset" | Agent calls `queue_device_recovery` and says a Console notice will ask you to set it up — **not** that it will re-provision by itself. Then cancel it: "cancel that recovery" |
| C3 | Chat: "What recovery actions are pending?" | Agent calls `list_device_recovery`, reports the queued one |
| **D. Real factory reset → setup (DESTRUCTIVE — needs go-ahead + the live device)** | | |
| D1 | Factory-reset the Q3538, then click **Queue setup notice** (or chat-queue it) | Pending action armed |
| D2 | Wait for the next health sweep (~60s) with the monitor enabled | Sweep fires the pending action **once**; the Console strip shows *"Factory-reset — onboard it to set it up"*; the audit row names the notice (`notice_id`) and is attributed to the approver + "deferred-trigger". Nothing was written to the device |
| D3 | Click **Review in chat** on that notice, then approve the card the chat raises | The chat calls `onboard_device`; after approval the device has **two** accounts — `root` with the fleet root password (log in with it by hand to check) and `admz` — and flips to `online`; the notice closes |
