...
class TestPerDomainModules:
    ...
    def test_tasks_module_has_one_tool(self):
        from admz.mcp.tools import tasks
        names = {t.name for t in tasks.TOOLS}
        assert names == {"list_tasks"}

    def test_audit_module_has_one_tool(self):
        from admz.mcp.tools import audit
        names = {t.name for t in audit.TOOLS}
        assert names == {"search_audit_log"}
    ...
    def test_drift_review_module_has_the_review_and_notice_tools(self):
        """ADR-0070's four review tools, then ADR-0071's two notice tools, in
        ...
        assert [t.name for t in drift_review.TOOLS] == [
            ...
            "list_config_ignore_rules", "list_notices", "dismiss_notice",
        ]
...
class TestMigratedToolsAggregate:
    def test_migrated_tools_count(self):
        ...
        # create_action_rule, delete_action_rule) + 17 (demos, ADR-0046/47 +
        ...
        # + 2 (drift_review: list_notices, dismiss_notice, ADR-0071)
        # = 48
        assert len(MIGRATED_TOOLS) == 48
