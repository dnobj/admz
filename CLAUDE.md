...
- **Liveness needs a heartbeat, not a handle — check `live` AND `lastActivity`.** On 2026-08-05 two sessions died mid-task and were reported as "still building" for hours, because progress was inferred from the number of uncommitted files in their worktrees — a number that does not change when a session stops. `live: false` settles it, and `read_session` shows what the last turn actually did. **But `live: true` settles nothing.** On 2026-08-16 a loop reported `live: "pid 88588 (claude-desktop)"` with a `lastActivity` six hours old: a session open in the desktop app holds its process indefinitely, so the pid means an app has it loaded, not that a turn is running. That reading was passed to the owner as "your loops are producing real work" and was wrong within the hour. Judge `lastActivity` against the cadence the session is supposed to have — a loop that ticks hourly and last moved six hours ago is stopped, whatever its pid says. A handle is an artifact and outlives the work exactly like an uncommitted file does. Related: `continue_session` returning `status: "running"` means *queued*, not *accepted* — a dispatch rejected by the fork-guard fails 90 ms later and looks identical unless `get_job` is checked.
...
**Install (Claude Code):** the skill lives at `C:\admz\.claude\skills\tick\SKILL.md` —
**beside the ledger, not in the repo**, because sessions run rooted at `C:\admz` and skills are
...
| **Session liveness** | `mcp__sy__list_sessions` — **`live` AND `lastActivity` together** · `mcp__sy__read_session` to see what a stalled session last did | **Never infer progress from a file count** — it does not change when a session dies. **Nor from `live` alone**: a session open in the desktop app reports a pid indefinitely, so `live: true` with a stale `lastActivity` is a stopped session, not a working one. `continue_session` returning `status: "running"` means *queued*, not *accepted*; confirm with `get_job`. |
...
- **The confirmation gate** (ADR-0034): risk → level (`none` / `llm_confirm` / `url_only` / `url_and_password`), single-sourced in `admz/confirm_policy.py` (`_DEFAULT_CONFIRMATION_LEVELS`, with per-fleet overrides) and resolved through `operations.resolve_confirmation`. `confirm_store` re-exports the table but no longer defines it. Capabilities may change *who may approve*; they never remove a gate. **A risk class the table does not know fails CLOSED** to `UNKNOWN_RISK_CONFIRMATION` (#397) — it used to fall back to `none`, which meant an unrecognised `risk_level` from the separately-pinned catalog ran with no confirmation at all. `tests/test_risk_vocabulary.py` holds the pinned catalog's vocabulary against the table, so a pin bump that introduces a new risk word fails CI rather than surprising an operator at runtime. **One severity scale** (`LEVEL_STRICTNESS`, #456): the plan engine's raise-only step floor compares *effective* confirmation levels through `operations.resolve_confirmation` — there is no second severity table, a declared word never softens a catalog word (an operator's stricter override included), and an unknown or empty catalog word resolves to the fail-closed level on every path.
