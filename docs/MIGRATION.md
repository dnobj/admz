...
open http://localhost:4242/api/docs
```

The REST API mirrors the MCP surface under the `/api/...` prefix (devices,
accounts, catalog, plans, snapshots, drift, discovery, schedules). See the
live OpenAPI reference at `/api/docs` and the route groups in the
[README](../README.md#as-a-web-service).

### 2. MCP Server Integration

v2 includes a full MCP server (44 tools) for AI agent integration:
...
The MCP surface covers device/account CRUD, the catalog-in-the-loop tools,
gated execution, multi-step plans, snapshots/restore/drift, discovery,
provisioning, firmware, scheduling, and fleet health. (Note: there is no
`get_credentials` tool — passwords never enter the LLM's context; the
executor uses stored credentials internally.) See
[MCP_TOOLS_REFERENCE.md](MCP_TOOLS_REFERENCE.md) for the full reference and
[MCP_INTEGRATION.md](MCP_INTEGRATION.md) for integration notes.
