...
convention rather than a control; it is now enforced.

This refusal does **not** by itself keep the password out of the MCP audit row.
An earlier version of this paragraph claimed it did; that was wrong (#217).
Tool arguments are sanitized *before* dispatch and recorded whatever the
handler decides, so a refused call still carried the plaintext. Masking it is
FR-SEC-014's job; this gate only stops the value being stored.
...
_As built:_ every sentence above is asserted in `tests/test_mcp_drift_tools.py`. The order after
`delete_devices` is `get_drift_review`, `revert_drift`, `ignore_config_keys`,
`list_config_ignore_rules`, `list_notices`, `dismiss_notice`. `check_drift` points review work at
`get_drift_review`. `list_notices` says a notice is attention, not an action
(`tests/test_notices_chat.py`). `dismiss_notice` says to use it only when the user says so.
...
`admz/mcp/server.py` registers the whole tool surface in a single class. The
