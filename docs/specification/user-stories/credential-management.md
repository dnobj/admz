...
**As a** security-conscious operator, **I want** plaintext credential retrieval **disabled by default** on both the MCP and REST surfaces **so that** a hostile LLM prompt cannot exfiltrate device passwords.

**Acceptance criteria:**
1. The MCP `get_credentials` tool is filtered out of `list_tools()` unless `fleet_settings["tool_get_credentials_enabled"] == "true"`.
2. With the flag off, calling the tool returns `{success: false, error: "ToolDisabled", ...}`.
3. The REST endpoint `GET /api/devices/{id}/credentials` returns HTTP 403 with a hint pointing at `/confirm-settings` when the flag is off.
4. The flag itself is **protected**: `set_fleet_setting("tool_get_credentials_enabled", …)` from MCP is rejected. Only the web UI at `/confirm-settings` can change it.
