...
**Acceptance criteria:**
1. `compute_upgrade_path(current="9.20.1", target="12.8.54")` returns the ordered list:
   `["9.80 (LTS)", "10.12 (LTS)", "11.11 (LTS)", "12.8.54"]`.
...
**Acceptance criteria:**
1. `list_cached_firmware()` returns `{count, files: [{model, version, file_name, file_path, file_size}, ...]}`.
2. Models are normalized so `P3245-V` and `AXIS P3245-V` map to the same entry.
...
**Acceptance criteria:**
1. `execute_operation(device_id, operation_id="firmwaremanagement.cgi:upgrade", params={"file_path": "~/.admz/firmware/P3245-V-12.8.54.bin"})` initiates the upgrade.
2. The operation is classified `dangerous` in the catalog — `confirm_token` flow applies. Operator must explicitly approve.
3. The executor uses multipart upload with an extended timeout (firmware uploads take minutes).
4. The catalog operation declares `response.expect_timeout: true` — the device reboots mid-upgrade, the executor's TimeoutException handler converts that to a success with warning ("device rebooting, expected").
5. The operator should call `firmwaremanagement.cgi:status` afterwards (read-only) to confirm the new version.
