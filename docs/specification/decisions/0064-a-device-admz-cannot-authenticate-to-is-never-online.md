...
One onboarding pass makes at most **3 entries × 2 ops = 6 credentialed operations** — a wrong entry costs two, the primary op and its corroborator (#149/#150). At the wire that is **up to 12 credentialed sends**: the executor re-sends an op once when the 401 challenge names a different auth method than the device profile (`vapix.py:693-696`), and onboarding hands every entry the same profile. Each Digest op also costs one *unauthenticated* challenge round-trip — whether Axis counts that as a failed login is exactly what decision 7 measures. The pass stops on first success and breaks on a `None` (unreachable) answer.

The bound is enforced **in the loop, not only at storage**. FR-CRED-013 chose a storage-time cap so the settings page could never show six credentials while ADMZ tried three; that argument survives — `describe()` already separates *stored* from *in use* — but the CLI writer (`python -m admz settings set entry_credentials`) bypasses the cap because `_parse` never truncates, so today the loop is unbounded by what one command can store. The bound is the cheap safety half and ships first (slice C, no measurement needed). `attempt_order` sorting by most-recently-successful — with no history the legacy pair first, today's behaviour kept as the control — is the ceremony half: **slice F does not merge until the lockout behaviour has been measured on a spare Axis unit**, the measurement ADR-0061 asked for and nobody ran; FR-CRED-013 records the result, or its absence, in words.
...
`admz/onboarding.py:369-394` (`MAX_ATTEMPTS_PER_PASS = MAX_STORED`; keep the `None` break), the stale module docstring in `admz/entry_credentials.py:5-6`. Docs: FR-CRED-013 (the bound, stated at wire level). Tests: an onboarding test counting executor *requests* (three wrong entries → six operations, twelve sends with the method-relearn re-send scripted; success at entry 2 → four operations), `tests/test_entry_credentials.py:133` unchanged.
...
`admz/provisioning.py:180` (`allow_fleet_default=False`). Docs: FR-CRED-007. Tests: `tests/test_provisioning.py:91` inverted; `:102`, `:184` unchanged. The `reprovision` detection handler already passes `allow_fleet_default=False`, so deferred reprovision does not change.
...
| 17 | One pass makes at most 2 ops per entry, 12 sends at the wire, and stops on first success | onboarding test counting executor *requests* (`_open_and_send`), with the method-relearn re-send scripted: three wrong entries → 6 operations / 12 sends; success at entry 2 → 4 operations | mutation: a second confirm per entry, or a third send per op → fails |
