...
> **As built, 2026-09-23.** For nine days the "defer" was only a refusal. The handler called `provision_factory_default(attended=False)`, every fired task failed, and the chat still offered "queue a re-provision". The attended flow now exists: a fired task raises a `setup` notice in the Console, its review sends the chat to `onboard_device` behind the usual approval, and a successful onboarding closes it ([ADR-0071's amendment](0071-a-task-raises-a-notice-the-console-delivers-it.md#amendment-2026-09-23--a-third-kind-setup)). The handler never reaches provisioning.
