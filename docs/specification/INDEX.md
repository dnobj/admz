...
- **[ui-deferred.md](ui-deferred.md)** — the register of UI surfaces that are *deliberately* absent (Org switching, Site management, discovery page, drift dashboard, bulk edit, …). Each row points at the requirement that owns the deferral and the `status: future` issue that tracks it. Read by the autonomous UI/UX audit loop so a decision is never re-reported as a gap; anything that looks unfinished and is **not** listed here is an ordinary finding.
...
- [0063 — Capability knowledge is local-first: probe once, record, revalidate on change; the atlas advises and never suppresses a probe](decisions/0063-capability-knowledge-is-local-first.md) ✅ — **Accepted; shipped (#454/#455/S3), live-verified.** The drift audit probed `sip:getSIPAccounts` on a PoE switch and logged an ERROR every hour: seven API-backed facets match every device because `api_family` is never written, the engine drops a failed read silently and records the facet `success=True, {}`, and nothing asks whether a device has an API before probing it. The obvious fix — consult the atlas, skip its negatives — was the first draft and is **wrong on the data**: the 2026-02 snapshots are legacy-only and lack every DCA-only id, so an exact-firmware match returns `action-rules: false`; the tie-break prefers partial captures; ADMZ passes `firmware_version` where the resolver reads `firmware`. A wrong skip costs a facet its coverage silently and forever; a wasted probe costs one request. So: one local `device_capabilities` store written by the audit's own reads, classified with the same-cycle readability control (the `param.cgi` dump succeeded ⇒ a specific op failing is evidence about that op — the rule that catches the T8516's `httpx.ReadError`, which a naive transport-means-no-record rule would have excluded forever); unknown means probe; absent rows expire in 7 days as the cadence; firmware-keyed so an upgrade invalidates with no code. Facets report `ok | skipped | failed`; drift enumerates baseline facets and separates `facets_absent` (drift, no DriftField) from `facets_unverified` (not drift — closes the transient-failure-reads-as-drifted bug). Survey is for everyone and writes positives locally; contributing to the atlas stays behind `survey.contributor`, including Run-now (ADR-0030 amended). Firmware change becomes an event at the health sweep's existing-but-discarded delta. Answers #123's attestation-home question: local table (#357, #123, #428)
...
- [0062 — A plan approval is an envelope, not a step list](decisions/0062-approve-an-envelope-not-a-step-list.md) 📋 — **Proposed, docs-only.** The operator wants the console to plan a multi-step goal, approve everything once up front, then execute — with the plan free to change. The last two conflict: plan-then-execute's security property is **control-flow integrity** (untrusted content may affect data flow but cannot hijack the sequence, because the sequence is fixed before untrusted input is touched), and a plan that changes after approval means the operator approved a plan that no longer exists — with a device response, the untrusted input, the most likely cause of the change. Re-approving each change is correct and unusable: a discover-then-act plan across eleven devices would prompt on every differing read, which is the gate-fatigue ADR-0034 already names. Resolves it by approving a **device set + risk ceiling + operation set** that may only ever NARROW; any step outside stops the plan and re-gates. ADMZ can do this honestly where most systems cannot — the atlas carries `risk_level` per operation and `execute_gated_plan` already computes the aggregate, so the envelope is derived from existing data rather than a new abstraction. Deliberately leaves `condition:` (#439) outside, since a conditional step is already inside the approved plan and narrows what runs. Notes that shipping this alone changes nothing an operator would see, because #438 — the chat never plans — is a separate defect (#440, #438, #439)
...
- [0061 — Fleet credentials get you in; ADMZ's own per-device account keeps you in](decisions/0061-entry-credentials-and-the-admz-account.md) 🚧 — **Accepted; slices 1–3 shipped 2026-08-17 (#446/#447/#449); the remainder is re-planned in ADR-0064.** One credential does two unrelated jobs: authenticating to a device ADMZ has never met, and being the standing key for the next two years. `onboarding.py:229` conflates them by storing the fleet pair *as* the ongoing credential, so a single shared password becomes the key to every device onboarded that way. Measured on production: `default_username` is `operator` and **none of the nine stored device accounts use it** (eight `root`, one `admz`) — so the fleet pair as configured resolves nothing and capture has been doing all the work. Splits the jobs: entry credentials become a **list** of `(username, password)` pairs used only to get in, and ADMZ creates its own `admz` administrator account per device with a generated password as the ongoing credential. Dissolves rather than splits `q_d7314a51`'s compromise — shared is right for entry (you cannot have a per-device secret for a device you have never met), generated is right for ongoing. Changes FR-CRED-007's password-source ordering, which until ADR-0064 slice E preferred the shared secret exactly where it is least appropriate. Carries four costs explicitly: account creation on a **non**-factory-defaulted device is a `pwdgrp.cgi:add-user`/`group=root` write ADR-0059's analysis did not cover; N entry credentials is N failed authentications and Axis brute-force behaviour varies by model; ADMZ must **never** delete the credential it came in on, because that is the only way back after a database loss (making #405's encryption a prerequisite); and an `admz` account outlives the device's removal from ADMZ, which is `q_70025d93`'s orphan problem through a new door. Existing devices are not migrated — creating accounts on nine live devices as a deploy side effect is a decision, not a consequence (`q_d7314a51`, #327, #405)
- [0064 — A device ADMZ cannot authenticate to is never `online`: credential state is resolved once at registration, projected by health, and re-opened by the operator](decisions/0064-a-device-admz-cannot-authenticate-to-is-never-online.md) ✅ — **Accepted; slices A–E shipped 2026-09-06 (#471, #472, #473, #474, #476); F waits for the lockout measurement (decision 7).** An A1210 registered through the MCP discovered-device path with no account row read `online` on every surface for seven hours, until a baseline capture happened to need a password (#443): the sweep catches the missing-credentials lookup and falls back to a TCP connect, which files `online`; KL-HLT-004 had declined to reclassify that because "awaiting capture" sounded short. Decides that `no_credentials` is a health **status** (not a field — every consumer of the enum needs a case, and a two-variable rule is the #357 failure shape): settled, attention, stamps reachability, never fires `on_online`. Registration always leaves a re-openable capture path (the discovered-device path opens a session like `register_device`; the device page gets **Enter credentials** for `no_credentials` and `auth_failed`), and the sweep classifies and never resolves — re-running onboarding from a sweep would spray every entry credential every 60 s and reach the account-write gate unattended. Plans #443 and the remainder of #411 as one state machine: ADR-0061 turned out to be three-fifths shipped (#446/#447/#449), with no operator writer for the entry list and the lockout measurement never run; slices C–F are the per-pass attempt bound (six credentialed operations, up to twelve sends), the promote checkbox, FR-CRED-007's generated-wins ordering, and most-recently-successful ordering (waiting on the lockout measurement). A factory-default unit without an account is `needs_setup`, not `no_credentials`: the credential-less tier asks `systemready` unauthenticated first. Two adjacent exposures get their own issues: the sweep's three failed authentications a minute on an `auth_failed` device, and drift reporting a credential-less device as unreachable (#443, #411)
...
- [Read-only fleet account sweep](../plans/fleet-account-sweep.md) — finding temp device accounts that predate #315's persistence fix. **Authorized** (`q_70025d93`) and not yet run. Blocked on a catalog change: the only candidate read op is an unverified `v2beta` REST call that older firmware will not have, so the enumeration path the authorization assumed cannot be relied on across the fleet. Should share a production maintenance window with #165, which needs the same atlas reinstall.
