...
| **Network discovery page** — run a scan and review results in the browser | No route, template or nav entry; only `POST /api/discovery/scan` and the chat tool. The Settings card's *Run discovery* control currently links to the API docs. The console gains a partial surface through ADR-0072 (planned): a scan run from chat renders as a table whose Add button is the approval. A standalone page, periodic scans and dismissals are still open. Direction is being designed by the owner | [discovery.md](requirements/discovery.md); [#404](https://github.com/dnobj/admz/issues/404) (`status: planning`) | #404 |
